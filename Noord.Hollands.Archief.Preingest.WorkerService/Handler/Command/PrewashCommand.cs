...
{
    /// <summary>
    /// Concrete class command for calling the client to do a prewash action.
    /// </summary>
    /// <seealso cref="Noord.Hollands.Archief.Preingest.WorkerService.Handler.AbstractPreingestCommand" />
    public class PrewashCommand : AbstractPreingestCommand
    {
        public override ValidationActionType ActionTypeName => ValidationActionType.PrewashHandler;
        /// <summary>
        /// Initializes a new instance of the <see cref="PrewashCommand"/> class.
        /// </summary>
        /// <param name="logger">The logger.</param>
        /// <param name="webapi">The webapi.</param>
        public PrewashCommand(ILogger<PreingestEventHubHandler> logger, Uri webapi) : base(logger, webapi) { }
        ...
        public override void Execute(HttpClient client, Guid currentFolderSessionId, Settings settings)
        {
            TryExecuteOrCatch(client, currentFolderSessionId, (id) =>
            {
                ...
                api.PrewashMetadataFilesAsync(id).GetAwaiter().GetResult();
            });
        }
    }
}
