...
namespace Noord.Hollands.Archief.Preingest.WorkerService.Handler.Command
{
    ...
    public class BinariesPronomMutationCommand : AbstractPreingestCommand
    {
        ...
        public BinariesPronomMutationCommand(ILogger<PreingestEventHubHandler> logger, Uri webapi) : base(logger, webapi) { }
        ...
        public override void Execute(HttpClient client, Guid currentFolderSessionId, Settings settings)
        {
            TryExecuteOrCatch(client, currentFolderSessionId, (id) =>
            {
                ...
                api.UpdateMetadataFilesWithPRONOMAsync(id).GetAwaiter().GetResult();
            });
        }
    }
}
